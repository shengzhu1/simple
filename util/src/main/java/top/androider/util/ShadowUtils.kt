...
object ShadowUtils {
    ...
    class Config {
        ...
        fun apply(src: Drawable?): Drawable {
            var src = src
            if (src == null) {
            ...
            drawable.addState(
                ...
                ShadowDrawable(
                    src, shadowRadius, shadowSizeNormal,
            ...
            drawable.addState(
                StateSet.WILD_CARD,
                ShadowDrawable(
                    src, shadowRadius, shadowSizePressed,
    ...
    class ShadowDrawable(
        ...
        private fun drawShadow(canvas: Canvas) {
            ...
            val shadowScaleHorizontal: Float =
                if (shadowOffset == 0f) 1F else shadowOffset / (shadowOffset + shadowOffsetHorizontal)
            val shadowScaleTop: Float =
                if (shadowOffset == 0f) 1F else shadowOffset / (shadowOffset + shadowOffsetTop)
            val shadowScaleBottom: Float =
                if (shadowOffset == 0f) 1F else shadowOffset / (shadowOffset + shadowOffsetBottom)
            ...
            saved = canvas.save()
            canvas.translate(
                mContentBounds.right - shadowOffset,
                mContentBounds.bottom - shadowOffset
            )
            ...
            saved = canvas.save()
            canvas.translate(
                mContentBounds.left + shadowOffset,
                mContentBounds.bottom - shadowOffset
            )
            canvas.scale(shadowScaleHorizontal, shadowScaleBottom)
            canvas.rotate(270f)
            ...
            if (isCircle) {
                ...
                if (shadowRadius > 0f) {
                    ...
                    mCornerShadowPaint.shader = RadialGradient(
                        ...
                        0F,
                        shadowRadius,
                        intArrayOf(0, mShadowStartColor, mShadowEndColor),
                        floatArrayOf(0.0f, startRatio, 1.0f),
        ...
        private fun buildComponents(bounds: Rect) {
            ...
            mContentBounds[bounds.left + mRawMaxShadowSize, bounds.top + verticalOffset, bounds.right - mRawMaxShadowSize] =
                bounds.bottom - verticalOffset
            wrappedDrawable!!.setBounds(
                mContentBounds.left.toInt(), mContentBounds.top.toInt(),
                mContentBounds.right.toInt(), mContentBounds.bottom.toInt()
            )
            buildShadowCorners()
        }
        ...
        init {
            mShadowEndColor = mShadowStartColor and 0x00ffffff
