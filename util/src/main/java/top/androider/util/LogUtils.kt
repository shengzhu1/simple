...
import top.androider.util.AppUtils.appVersionCode
import top.androider.util.AppUtils.appVersionName
...
class LogUtils private constructor() {
    ...
    class Config internal constructor() {
        ...
        var saveDays = -1 // The save days of log.
            private set
        private val mProcessName: String? = currentProcessName
        ...
        init {
            if (isSDCardEnableByEnvironment
                ...
                app!!.getExternalFilesDir(null).toString() + FILE_SEP + "log" + FILE_SEP else {
                defaultDir = app!!.filesDir.toString() + FILE_SEP + "log" + FILE_SEP
            }
        }
    }
    ...
    internal object LogFormatter {
        ...
        fun object2String(`object`: Any, type: Int = -1): String? {
            ...
            if (`object` is Throwable) return `object`.getFullStackTrace ()
        ...
        private fun object2Json(`object`: Any): String? {
            return if (`object` is CharSequence) {
                ...
                getGson4LogUtils()!!.toJson(`object`)
            } catch (t: Throwable) {
                `object`.toString()
            }
        }
    ...
    companion object {
        ...
        private val I_FORMATTER_MAP = HashMap<Class<*>?, IFormatter<*>>()
        ...
        val logFiles: List<File>
            get() {
                ...
                logDir.listFiles { dir, name -> isMatchLogFileName(name) }?.takeIf { it.isNotEmpty() }?.let {
                    list.addAll(it)
                }
                return list
            }
        ...
        private fun formatObject(`object`: Any?): String {
            ...
            if (!I_FORMATTER_MAP.isEmpty()) {
                val iFormatter:IFormatter<Any?> = I_FORMATTER_MAP[getClassFromObject(`object`)] as IFormatter<Any?>
                if (iFormatter != null && null != `object`) return iFormatter.format(`object`)
            }
        ...
        private fun print2File(type: Int, tag: String, msg: String) {
            ...
            val fullPath = (config.dir + config.filePrefix + "_"
                    + date + "_" +
