...
object ThreadUtils {
    ...
    @JvmStatic
    infix fun runOnUiThread(runnable: ()->Unit) {
    ...
    @JvmStatic
    infix fun runOnUiThread(runnable: Runnable) {
    ...
    @JvmStatic
    fun runOnUiThreadDelayed(runnable: ()->Unit, delayMillis: Long) {
    ...
    private fun getPoolByTypeAndPriority(type: Int, priority: Int): ExecutorService? {
        synchronized(TYPE_PRIORITY_POOLS) {
            ...
            if (priorityPools == null) {
                ...
                pool = priorityPools[priority]?:ThreadPoolExecutor4Util.createPool(type, priority).apply {
                    priorityPools[priority] = this
                }
            }
            return pool
        }
    ...
    class LinkedBlockingQueue4Util : LinkedBlockingQueue<Runnable> {
        ...
        override fun offer(runnable: Runnable): Boolean {
            return if (mCapacity <= size && (mPool?.takeIf { it.poolSize < it.maximumPoolSize }.isNotEmpty()?:false)) {
    ...
    internal class UtilsThreadFactory (
        prefix: String,
        priority: Int,
        isDaemon: Boolean = false): AtomicLong(), ThreadFactory {
